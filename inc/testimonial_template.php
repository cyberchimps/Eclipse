...
// frontend display
function eclipse_testimonial_render_display()
{
	global $post;
        // Set directory uri
	...
	if(is_page())
	{
		$eclipse_testimonial_title = get_post_meta( $post->ID, 'ir_testimonial_title', true );
	...
	{
		$eclipse_testimonial_title = cyberchimps_get_option('ir_testimonial_title');
	...
	if(  !empty($eclipse_testimonial_title)  ){
	...
	<div id="eclipse_testimonial_top">
		<?php if( !empty($eclipse_testimonial_title) ) { ?>


				<?php if(!empty($eclipse_testimonial_title)) { ?>
				<h2 class="eclipse_main_title">
					<?php echo $eclipse_testimonial_title; ?>
				</h2>
				<?php } ?>
      ...
      <section class="eclipse_slider_text_img">
        <div id="slider2" class="flexslider">
          <ul class="slides">
			<?php	foreach( $testimonials as $testimonial) {
				...
				<li class="col-md-12">
                                    <span class="eclipse_testimonial_text"><?php if(!empty($testimonial['text'])){echo $testimonial['text'];} ?></span>
					<?php if(!empty($testimonial['text']))
							{ ?>
							<hr class="after_testimonial_text"> </hr>
					<?php } ?>
					<div class="eclipse_testimonial_author">
                                                <?php  if(!empty($testimonial['client'])){echo $testimonial['client'];} ?>
					</div>
					<div class="eclipse_testimonial_abt_author">
	...
	<?php $slide_counters = 0; ?>
        <div id="carousel2" class="flexslider">
          <ul class="slides eclipse_carousel" >
			<?php	foreach( $testimonials as $testimonial ) {
